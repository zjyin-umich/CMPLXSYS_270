...
to set-income ;; sets income based of US 2014 income brackets, in 1000s of dollars
  ask turtles [
    let income-perc random-float 1
    (ifelse
      income-perc <= 0.1 [
        set income random 0.5
      ]
      income-perc <= 0.25 [
        set income 0.5 + random 1.3
      ]
      income-perc <= 0.5 [
        set income 1.3 + random 2.7
      ]
      income-perc <= 0.75 [
        set income 2.7 + random 5.3
      ]
      income-perc <= 0.9 [
        set income 5.3 + random 8.9
      ]
      ; else
      [  set income 8.9 + random 30 ]
      )
  ]
end
...
to tax-redistribute-even
  ;; taxing each turtle and determining new wealth after paying
  ask turtles [
    set tax-payout (income * income-tax-rate)
    set wealth (wealth - tax-payout)
  ]

  ;; redistributing taxes evenly across all turtles
  let total-taxes sum [tax-payout] of turtles
  let total-avail (total-taxes * (1 - govt-spending-rate)) ;; subtracts off the amount of money spent by governemnt
  let redistribute-value (total-avail / num-people) ;; distributes remaining funds back to the people
  ask turtles [ set wealth (wealth + redistribute-value) ]
end

to tax-redistribute-focused
  ;; taxing each turtle and determining new wealth after paying
  ask turtles [
    set tax-payout (income * income-tax-rate)
    set wealth (wealth - tax-payout)
  ...
  ;need to access bottom 50% of all turtles to evenly distribute to them
...
;; the above procedure is from the Wealth Distribution model
...
"" ""
PENS
"current" 5.0 1 -13840069 true "" "set-plot-y-range 0 5\nhistogram [ wealth ] of turtles"
